...
date: "August 27, 2019 - Revision 3"
output:
  html_document:
    theme: united
    toc: yes
...
2. We've re-calculated some of our point estimates using both the OncoLnc read data (MK2.Expression) and the TCGA-Extracted read data (MK2.Expression.Manual)

3. The original scatter plot showing differences in MK2 is restricted to only "Adenocarcinomas" within LUAD. We've applied that restriction to all the regression analyses as well

4. Some small clean-ups to the code

5. We've re-analyzed the proportion dead by tertiles of MK2 (instead of quantiles)

6. One advantage of downloading TCGA data is that we also happened to grab MK2 transcript levels from 50 samples with normal tissue controls, thus allowing us to calculate fold change in MK2 transcript levels between normal and tumor in matched samples. 
...
summary(glm(death_at_1year ~ MK2.Expression.Manual, data=MK2_CoxPH_quantile[MK2_CoxPH_quantile$ajcc_pathologic_stage_combined=="Early Stage", ], family=binomial(link="logit")))

# so you can see above that it doesn't matter which MK2 expression level you use - the point estimate is essentially the same 
...
summary(glm(death_at_1year ~ MK2.Expression.Manual, data=MK2_CoxPH_quantile[MK2_CoxPH_quantile$ajcc_pathologic_stage_combined=="Late Stage", ], family=binomial(link="logit")))
...
As we noted in the code chunk itself, when we repeat the regressions using the manually derived expression data, the point estimates are essentially the same. 
...
summary(glm(death_at_1year ~ MK2.Expression.Manual+smoking+age_at_diagnosis+gender, data=MK2_CoxPH_quantile[MK2_CoxPH_quantile$ajcc_pathologic_stage_combined=="Early Stage", ], family=binomial(link="logit")))
...
Again, similar to above we run the multi-variate regression using expression data obtained from 2 different sources and get the same point estimate. 
...
```
## Model 1 in paper: Cox PH model R censored at one year, stratified by early late, MK2 as dichotomous (top 1/3 vs. bottom 2/3)
...
ggsurvplot(km_early, xlim=c(0,365), conf.int = TRUE, break.x.by=100, pval=TRUE, font.y=14,font.x=14,font.tickslab=14, ggtheme=theme_bw())
...
ggsurvplot(km_late, xlim=c(0,365), conf.int = TRUE, break.x.by=100, pval=TRUE, font.y=14,font.x=14,font.tickslab=14, ggtheme=theme_bw())
...
ggplot(MK2efx, aes(x=stage, y=fit, group=MK2_Expression_topQ, color=MK2_Expression_topQ, ymin=lower,ymax=upper))+geom_point(position=position_dodge(width=0.2))+geom_errorbar(width=0, position=position_dodge(width=0.2))+theme_bw(base_size = 14)+ylab("Predicted  Probability of death at one year")
...
# These data are plotted elsewhere. 
